##Tyler Sperle - 04/19/2022

import xml.etree.ElementTree as ET #These are all of our imported modules to help us utilize netconf and xml
import xmltodict
...
intIP = input("Please enter a new IP address for GigabitEthernet2: ") #This asks the user to enter an IP address for the interface
print("")

if validateIP(intIP) == True: #If the supplied IP address is valid, it will proceed
    intMask = input("Please enter a new subnet mask for GigabitEthernet2: ") #This asks the user to enter a subnet mask for the interface
    print("")
    
    if validateMask(intMask) == True: #If the supplied subnet mask is valid, it will proceed
        updatedInt = updateAddr(router, intIP, intMask)
        updatedIntList = netCONF(router)
